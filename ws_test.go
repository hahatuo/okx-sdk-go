...
func TestWSReconnectResubscribes(t *testing.T) {
	...
	if got := client.Reconnects(); got == 0 {
		t.Fatal("expected reconnect count to increase")
	}
}
...
func TestWSLoginBeforeConnectReturnsNotConnectedAndCleansPending(t *testing.T) {
	...
	if pendingCount != 0 {
		t.Fatalf("pending login waiters were not cleaned up: %d", pendingCount)
	}
}
...
func TestWSDispatchesOperationByIDAndOp(t *testing.T) {
	...
	client.ops.Store(operationPendingKey(1, "order"), respCh)
	defer client.ops.Delete(operationPendingKey(1, "order"))

	client.dispatch([]byte(`{"id":"1","op":"order","code":"0","msg":"","data":[{"ordId":"10","clOrdId":"c1","sCode":"0","sMsg":""}],"inTime":"1","outTime":"2"}`))
...
func TestWSMethodsBeforeConnectReturnNotConnectedAndCleanUp(t *testing.T) {
	...
	var opCount int
	client.ops.Range(func(_, _ any) bool {
		opCount++
		return true
	})
	if opCount != 0 {
		t.Fatalf("pending operations were not cleaned up: %d", opCount)
	}
...
func TestWSConnectionLossFailsPendingControlPlane(t *testing.T) {
	...
	respCh := make(chan WSOperationResponse, 1)
	client.pendingMu.Lock()
	client.pending[controlPendingKey("login", "")] = ackCh
	client.pendingMu.Unlock()
	client.ops.Store(operationPendingKey(1, "order"), respCh)
	defer func() {
		client.pendingMu.Lock()
		delete(client.pending, controlPendingKey("login", ""))
		client.pendingMu.Unlock()
	}()
	defer client.ops.Delete(operationPendingKey(1, "order"))

	client.failPending(ErrWSConnectionLost)

	select {
	case err := <-ackCh:
		if !errors.Is(err, ErrWSConnectionLost) {
			t.Fatalf("ack err = %v", err)
		}
	default:
		t.Fatal("pending ack was not failed")
	}

	select {
	case resp := <-respCh:
		if !errors.Is(resp.Err, ErrWSConnectionLost) {
			t.Fatalf("operation err = %v", resp.Err)
		}
	default:
		t.Fatal("pending operation was not failed")
	}
}

func TestWSControlAckDispatchesBySemanticKey(t *testing.T) {
	client := NewWSClient(Public)
	ack1 := make(chan error, 1)
	ack2 := make(chan error, 1)
	key1 := controlPendingKey("subscribe", subArg{Channel: "tickers", InstID: "ETH-USDT"}.key())
	key2 := controlPendingKey("subscribe", subArg{Channel: "tickers", InstID: "BTC-USDT"}.key())
	client.pendingMu.Lock()
	client.pending[key1] = ack1
	client.pending[key2] = ack2
	client.pendingMu.Unlock()

	// Dispatch a subscribe ACK for BTC-USDT — should deliver to ack2 only
	client.dispatch([]byte(`{"event":"subscribe","code":"0","arg":{"channel":"tickers","instId":"BTC-USDT"}}`))

	select {
	case err := <-ack2:
		if err != nil {
			t.Fatalf("ack2 err = %v", err)
		}
	default:
		t.Fatal("ack2 was not delivered")
	}
	select {
	case err := <-ack1:
		t.Fatalf("ack1 received wrong ack: %v", err)
	default:
	}

	// Dispatch an error ACK for ETH-USDT — should deliver to ack1
	client.dispatch([]byte(`{"event":"error","code":"60018","msg":"Wrong URL or channel","arg":{"channel":"tickers","instId":"ETH-USDT"}}`))

	select {
	case err := <-ack1:
		var apiErr *APIError
		if !errors.As(err, &apiErr) || apiErr.Code != "60018" {
			t.Fatalf("ack1 err = %#v", err)
		}
	default:
		t.Fatal("ack1 was not delivered")
	}
}
...
func TestSubscriptionKeyIncludesStructuredFields(t *testing.T) {
	...
	if subKey(WSChannelBooks5, "BTC-USDT") != (subArg{Channel: WSChannelBooks5, InstID: "BTC-USDT"}).key() {
		t.Fatal("legacy subKey wrapper diverged")
	}
}
