...
func (c *WSClient) SubscribeLocalOrderBookHandle(ctx context.Context, channel, instID string, depth int, handler WSLocalOrderBookHandler) (*WSSubscription, error) {
	...
	return c.SubscribeOrderBookHandle(ctx, channel, instID, localOrderBookHandler(
		instID,
		depth,
		OrderBookDepthFromChannel(channel),
		handler,
	))
}

func localOrderBookHandler(instID string, viewDepth, retainedDepth int, handler WSLocalOrderBookHandler) WSTypedHandler[OrderBook] {
	book := newLocalOrderBook(instID, viewDepth, retainedDepth)
	return func(msg WSTypedMessage[OrderBook]) {
		out := WSLocalOrderBookMessage{
			Arg:    msg.Arg,
			Action: msg.Action,
			Book:   book,
		}
...
func subscribeTyped[T any](ctx context.Context, c *WSClient, arg WSChannelArg, handler WSTypedHandler[T]) error {
	_, err := subscribeTypedHandle(ctx, c, arg, handler)
	return err
}
...
func subscribeTypedHandle[T any](ctx context.Context, c *WSClient, arg WSChannelArg, handler WSTypedHandler[T]) (*WSSubscription, error) {
	...
	return c.SubscribeArgHandle(ctx, arg, func(msg Message) {
		handler(decodeTypedWSMessage[T](msg))
	})
}
